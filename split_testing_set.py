...
source_imgs_dir = './DRIVE/test/image'
source_masks_dir = './DRIVE/test/label'
target_imgs_dir = './DRIVE_patch/test/image'
target_masks_dir = './DRIVE_patch/test/label'
...
for filename in os.listdir( source_imgs_dir ):
	...
	for patch_idx in range( 1 , patch_number + 1 ):
		...
		target_imgs_filename = str( target_idx ) + '_testing.png'
		target_imgs_filepath = os.path.join( target_imgs_dir , target_imgs_filename )    #输出图像待保存地址
		target_masks_filename = str( target_idx ) + '_testing.png'
...
print("Successfully split the testing set.")
