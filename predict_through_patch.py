...
for x_idx in range( sample_time ):
    ...
    for y_idx in range( sample_time ):
        ...
        patch_img_array = np.array( patch_img )    #将图像转换为数组
        patch_img_array = cv2.equalizeHist( patch_img_array )    #对patch做直方图均衡化
        patch_img = Image.fromarray( patch_img_array )    #将数组还原为图像
